...
public class UserBlacklistController {
    ...
    @Operation(summary = "Retrieve user's blacklist",
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content),
            @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content),
            @ApiResponse(responseCode = "404", description = "User to be blacklisted not found", content = @Content)
    })
    ...
    public UserDto addToBlacklist(@RequestBody UserBlacklistRequestDto dto, @AuthenticationPrincipal UserDetails userDetails)
        ...
        UserEntity blocker = userEntityService.getUserByUserDetails(userDetails);
        return new UserDto(userEntityService.addToBlacklist(blocker, dto).getBlocked());
    }

    @Operation(summary = "Remove user from user's blacklist",
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "Bad request", content = @Content),
            @ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content),
            @ApiResponse(responseCode = "404", description = "User to be unblacklisted not found", content = @Content)
    })
    ...
    public void removeFromBlacklist(@PathVariable int userId, @AuthenticationPrincipal UserDetails userDetails) throws UserNotFoundException {
        UserEntity blocker = userEntityService.getUserByUserDetails(userDetails);
        userEntityService.removeFromBlacklist(blocker, userId);
    }
}
